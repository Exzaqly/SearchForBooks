...
const actions = {
  booksReceived: (books: BookType[]) =>
    ({ type: BOOKS_RECEIVED, payload: books } as const),
  setTotalResults: (totalResult: number) =>
    ({ type: SET_TOTAL_RESULTS, payload: { totalResult } } as const),
  setTotalPages: (totalPages: number) =>
    ({ type: SET_TOTAL_PAGES, payload: { totalPages } } as const),
  setSearchTerm: (searchTerm: string) =>
    ({ type: SET_SEARCH_TERM, payload: { searchTerm } } as const),
  ...
  toggleLoadMore: (loadMore: boolean) =>
    ({ type: TOGGLE_LOAD_MORE, payload: { loadMore } } as const),
  bookReceived: (book: BookType) =>
    ({ type: BOOK_RECEIVED, payload: book } as const),
}

const _getBooks = (): Thunk => async (dispatch, getState) => {
  try {
    const data = await booksAPI.getBooks(
      getState().books.searchTerm,
      getState().showing.category,
      getState().books.currentPage,
      getState().showing.sorting
    )
